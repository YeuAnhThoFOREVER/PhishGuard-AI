...
Gmail에서 이메일을 열면 **Anthropic Claude AI**가 자동으로 내용을 분석하여 피싱 여부를 판단하고, 사용자에게 **맞춤형 보안 체크리스트**를 제공하는 Chrome 확장 프로그램입니다.
...
| Anthropic Claude API | 4.6 Sonnet | 이메일 피싱 분석 |
...
- **Anthropic Claude API 키** — [https://console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys) 에서 발급
...
2. **Anthropic Claude API 키** 입력 (sk-ant-로 시작)
3. **"💾 저장"** 버튼 클릭
...
- 이메일 내용은 Anthropic Claude API에만 전달되며 저장되지 않음
